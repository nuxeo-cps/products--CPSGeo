...
class Reader111Test(unittest.TestCase):
    ...
    def test_reader(self):
        ...
        self.assert_(cap.getmapformats() == ('image/png', 'image/jpeg'), cap.getmapformats())
        self.assert_(cap.layersrs() == ('EPSG:4326', 'AUTO:42003'), cap.layersrs())
        self.assert_(cap.layernames() == ('global_mosaic', 'global_mosaic_base', 'us_landsat_wgs84', 'srtm_mag', 'us_overlays', 'us90_overlays', 'daily_terra', 'daily_aqua', 'modis', 'huemapped_srtm', 'srtmplus', 'worldwind_dem', 'us_ned', 'us_elevation', 'us_colordem'), cap.layernames())
        self.assert_(cap.layertitles() == ('WMS Global Mosaic, pan sharpened', 'WMS Global Mosaic, not pan sharpened', 'CONUS mosaic of 1990 MRLC dataset', 'SRTM reflectance magnitude, 30m', 'Progressive US overlay map, white background', 'MRLC US mosaic with progressive overlay map', 'Daily composite of MODIS-TERRA images ', 'Daily composite of MODIS-AQUA images ', 'Blue Marble, Global MODIS derived image', 'SRTM derived global elevation, 3 arc-second, hue mapped', 'Global 1km elevation, seamless SRTM land elevation and ocean depth', 'SRTM derived global elevation, 3 arc-second', 'United States elevation, 30m', 'Digital Elevation Map of the United States, DTED dataset, 3 second resolution, grayscale', 'Digital Elevation Map of the United States, DTED dataset, 3 second resolution, hue mapped'), cap.layertitles())
